...
summary: Antes de usar o SnapCenter para fazer backup de recursos do cluster PostgreSQL, você deve criar uma política de backup para o recurso ou grupo de recursos do qual deseja fazer backup.  Uma política de backup é um conjunto de regras que regem como você gerencia, agenda e mantém backups. 
---
...
.. Na seção Configurações de instantâneo, especifique as configurações de retenção para o tipo de backup e o tipo de agendamento selecionado na página *Tipo de backup*:
+
|===
...
NOTE: Você deve definir a contagem de retenção como 2 ou mais se planeja habilitar a replicação do SnapVault .  Se você definir a contagem de retenção como 1, a operação de retenção poderá falhar porque o primeiro Snapshot será o Snapshot de referência para o relacionamento SnapVault até que um Snapshot mais recente seja replicado para o destino.
...
Ver link:view-postgresql-cluster-backups-and-clones-in-the-topology-page.html["Visualize backups e clones relacionados a recursos do PostgreSQL na página Topologia"] .
...
Para obter mais informações sobre o SnapLock Vault, consulte Confirmar instantâneos para WORM em um destino de cofre
