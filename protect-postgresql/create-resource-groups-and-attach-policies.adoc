...
Por exemplo, se você adicionar RH como uma tag a vários grupos de recursos, poderá encontrar posteriormente todos os grupos de recursos associados à tag RH.
 ...
 a| 
Use formato de nome personalizado para cópia de instantâneo
 a| 
Marque esta caixa de seleção e insira um formato de nome personalizado que você deseja usar para o nome do instantâneo.

Por exemplo, customtext_resource group_policy_hostname ou resource group_hostname.  Por padrão, um registro de data e hora é anexado ao nome do instantâneo.
...
| Para este campo... | Faça isso... 
 ...
 a| 
Permitir tempo para esperar a conclusão da operação de instantâneo do Consistency Group
 a| 
Selecione *Urgente*, *Médio* ou *Relaxado* para especificar o tempo de espera para a conclusão da operação de instantâneo.
...
NOTE: Os pares de chave-valor personalizados são suportados para sistemas de plug-in Linux do PostgreSQL e não são suportados para clusters PostgreSQL registrados como um plug-in centralizado do Windows.
...
Selecione * SnapCenter sem consistência do sistema de arquivos*.
 ...
 a| 
Para inserir o comando a ser executado no host para criar cópias de instantâneos.
 a| 
Selecione *Outro* e insira o comando a ser executado no host para criar um instantâneo.
