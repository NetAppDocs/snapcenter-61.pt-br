...
+
link:../protect-postgresql/configure-gMSA-on-windows-server-2012-or-later.html["Configurar conta de serviço gerenciada de grupo no Windows Server 2016 ou posterior para PostgreSQL"]
...
NOTE: O plug-in para PostgreSQL é instalado no host do cliente PostgreSQL, e esse host pode estar em um sistema Windows ou Linux.
...
Caminho de instalação
 a| 
O plug-in para PostgreSQL é instalado no host do cliente PostgreSQL, e esse host pode estar em um sistema Windows ou Linux.
...
Adicionar todos os hosts no cluster
 a| 
Marque esta caixa de seleção para adicionar todos os nós do cluster.
