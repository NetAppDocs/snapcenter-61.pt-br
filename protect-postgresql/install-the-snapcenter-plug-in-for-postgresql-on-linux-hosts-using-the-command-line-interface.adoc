...
summary: 'Você deve instalar o plug-in SnapCenter para cluster PostgreSQL usando a interface de usuário (IU) do SnapCenter .  Se o seu ambiente não permitir a instalação remota do plug-in a partir da interface do usuário do SnapCenter , você poderá instalar o cluster do Plug-in para PostgreSQL no modo de console ou no modo silencioso usando a interface de linha de comando (CLI1).' 
---
...
+
https://imt.netapp.com/matrix/imt.jsp?components=121069;&solution=1259&isHWU&src=IMT["Ferramenta de Matriz de Interoperabilidade da NetApp"]
...
.Passos
. Copie o arquivo de instalação do pacote de plug-ins do SnapCenter para Linux (snapcenter_linux_host_plugin.bin) de C:\ProgramData\ NetApp\ SnapCenter\Package Repository para o host onde você deseja instalar o plug-in para PostgreSQL.
+
...
----
. Edite o arquivo /<diretório de instalação>/ NetApp/snapcenter/scc/etc/SC_SMS_Services.properties e adicione o parâmetro PLUGINS_ENABLED = PostgreSQL:3.0.
