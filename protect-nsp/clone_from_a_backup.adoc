...
sidebar: sidebar 
keywords: clone 
summary: Você pode usar o SnapCenter para clonar um backup.  Você pode clonar a partir do backup primário ou secundário.  Os recursos das operações de clonagem dependem do plug-in que você usa. 
---
= Clonar de um backup
:allow-uri-read: 
:icons: font
...
. Na página Locais, faça o seguinte:
+
|===
...
Se você quiser especificar um host diferente, selecione o host no qual o clone deve ser montado e o plug-in será instalado.
...
Insira um sufixo que será anexado ao nome do recurso recém-clonado.  O sufixo garante que o recurso clonado seja exclusivo no host.

Por exemplo, rs1_clone.  Se você estiver clonando para o mesmo host do recurso original, deverá fornecer um sufixo para diferenciar o recurso clonado do recurso original; caso contrário, a operação falhará.
...
Nome do iniciador
 a| 
Digite o nome do iniciador do host, que pode ser um IQDN ou WWPN.
...
Protocolo Igroup
 a| 
Selecione o protocolo Igroup.

|===
+

NOTE: A página de configurações será exibida somente se o tipo de armazenamento for LUN.
...
+
Comando de montagem para um volume ou qtree em uma máquina Linux: mount<VSERVER_NAME>:%<VOLUME_NAME_Clone /mnt>
...
PS C:\> Get-SmResourceGroup
    ...
    PolicyNames                 :
----
. Inicie uma operação de clonagem de um grupo de recursos de clonagem ou de um backup existente usando o cmdlet New-SmClone.
+
