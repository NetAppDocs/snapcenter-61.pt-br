...
summary: Você deve criar uma função de cluster ONTAP com privilégios mínimos para não precisar usar a função de administrador ONTAP para executar operações no SnapCenter. 
---
...
. No sistema de armazenamento, crie uma função e atribua todas as permissões a ela.
+
...
. Crie um usuário e atribua a função a ele.
+
