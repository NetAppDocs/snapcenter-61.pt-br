...
sidebar: sidebar 
keywords: SnapCenter 
summary: 'O SnapCenter usa a tecnologia Snapshot para fazer backup de recursos do sistema de arquivos do Windows que residem em LUNs, CSVs (volumes compartilhados de cluster), volumes RDM (mapeamento de dispositivos brutos), ALM (mapeamento de LUN assimétrico) em clusters do Windows e VMDKs baseados em VMFS/NFS (VMware Virtual Machine File System usando NFS).' 
---
...
NOTE: Se um sistema de arquivos do Windows contiver um banco de dados, fazer backup do sistema de arquivos não será o mesmo que fazer backup do banco de dados.  Para fazer backup de um banco de dados, você deve usar um dos plug-ins de banco de dados.
